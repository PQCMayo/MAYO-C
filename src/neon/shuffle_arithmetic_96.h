...
inline void mayo_3_P1_times_O_neon(const uint64_t *_P1, uint8x16_t *O_multabs, uint64_t *_acc){
    ...
    for (size_t r = 0; r < V_MAX; r++)
    {
        ...
        for (size_t k = 0; k < O_MAX; k+=2)
        {
            uint8x16_t t0 = (temp[3*k + 1] ^ (temp[3*k] >> 4)) & low_nibble_mask;
            uint8x16_t t1 = (temp[3*k + 3] ^ (temp[3*k + 2] >> 4)) & low_nibble_mask;
            uint8x16_t t2 = (temp[3*k + 5] ^ (temp[3*k + 4] >> 4)) & low_nibble_mask;
            acc[(3*r*O_MAX) + 3*k]     ^= temp[3*k]     ^ (t0 << 4);
            acc[(3*r*O_MAX) + 3*k + 1] ^= temp[3*k + 2] ^ (t1 << 4);
            acc[(3*r*O_MAX) + 3*k + 2] ^= temp[3*k + 4] ^ (t2 << 4);
            acc[(3*r*O_MAX) + 3*k + 3] ^= temp[3*k + 1] ^ t0;
            acc[(3*r*O_MAX) + 3*k + 4] ^= temp[3*k + 3] ^ t1;
            acc[(3*r*O_MAX) + 3*k + 5] ^= temp[3*k + 5] ^ t2;
        }
    }
}
...
inline void mayo_3_Ot_times_P1O_P2_neon(const uint64_t *_P1O_P2, uint8x16_t *O_multabs, uint64_t *_acc){
    ...
    for (size_t c = 0; c < O_MAX; c++)
    {
        ...
        for (size_t k = 0; k < O_MAX; k+=2)
        {
            ...
            uint8x16_t t1 = (temp[3*k + 3] ^ (temp[3*k + 2] >> 4)) & low_nibble_mask;
            uint8x16_t t2 = (temp[3*k + 5] ^ (temp[3*k + 4] >> 4)) & low_nibble_mask;
            acc[3*(k*O_MAX) + 3*c]      ^= temp[3*k + 0] ^ (t0 << 4);
            acc[3*(k*O_MAX) + 3*c + 1]  ^= temp[3*k + 2] ^ (t1 << 4);
            acc[3*(k*O_MAX) + 3*c + 2]  ^= temp[3*k + 4] ^ (t2 << 4);
            acc[3*((k+1)*O_MAX) + 3*c]     ^= temp[3*k + 1] ^ t0;
            acc[3*((k+1)*O_MAX) + 3*c + 1] ^= temp[3*k + 3] ^ t1;
            acc[3*((k+1)*O_MAX) + 3*c + 2] ^= temp[3*k + 5] ^ t2;
        }
    }
}
...
inline void mayo_3_P1P1t_times_O_neon(const uint64_t *_P1, const unsigned char *O, uint64_t *_acc){
    ...
    for (size_t r = 0; r < V_MAX; r++)
    {
        ...
        for (size_t k = 0; k < O_MAX; k+=2)
        {
            uint8x16_t acc0 = acc[3*(r*O_MAX) + 3*k    ];
            uint8x16_t acc1 = acc[3*(r*O_MAX) + 3*k + 1];
            uint8x16_t acc2 = acc[3*(r*O_MAX) + 3*k + 2];
            uint8x16_t acc3 = acc[3*(r*O_MAX) + 3*k + 3];
            uint8x16_t acc4 = acc[3*(r*O_MAX) + 3*k + 4];
            uint8x16_t acc5 = acc[3*(r*O_MAX) + 3*k + 5];


            uint8x16_t t0 = (temp[3*k + 1] ^ (temp[3*k] >> 4)) & low_nibble_mask;
            acc[3*(r*O_MAX) + 3*k] =     acc0 ^ temp[3*k] ^ (t0 << 4);
            acc[3*(r*O_MAX) + 3*k + 3] = acc3 ^ temp[3*k+1] ^ t0;

            uint8x16_t t1 = (temp[3*k + 3] ^ (temp[3*k + 2] >> 4)) & low_nibble_mask;

            acc[3*(r*O_MAX) + 3*k + 1] = acc1 ^ temp[3*k+2] ^ (t1 << 4);
            acc[3*(r*O_MAX) + 3*k + 4] = acc4 ^ temp[3*k+3] ^ t1;

            uint8x16_t t2 = (temp[3*k + 5] ^ (temp[3*k + 4] >> 4)) & low_nibble_mask;

            acc[3*(r*O_MAX) + 3*k + 2] = acc2 ^ temp[3*k+4] ^ (t2 << 4);
            acc[3*(r*O_MAX) + 3*k + 5] = acc5 ^ temp[3*k+5] ^ t2;
        }
    }
}
...
inline void mayo_3_Vt_times_L_neon(const uint64_t *_L, const uint8x16_t *V_multabs, uint64_t *_acc){
    ...
    for (size_t c = 0; c < O_MAX; c++)
    {
        ...
        {
            uint8x16_t t0 = (temp[3*k + 1] ^ (temp[3*k] >> 4)) & low_nibble_mask;
            acc[3*(k*O_MAX) + 3*c]     ^= temp[3*k] ^ (t0 << 4);
            acc[3*((k+1)*O_MAX) + 3*c] ^= temp[3*k+1] ^ t0;
...
inline void mayo_3_P1_times_Vt_neon(const uint64_t *_P1, uint8x16_t *V_multabs, uint64_t *_acc){
    ...
    for (size_t r = 0; r < V_MAX; r++)
    {
        ...
        {
            uint8x16_t t0 = (temp[3*k + 1] ^ (temp[3*k] >> 4)) & low_nibble_mask;
            acc[3*(r*K_MAX) + 3*k] ^= temp[3*k] ^ (t0 << 4);
            acc[3*(r*K_MAX) + 3*k + 3] ^= temp[3*k+1] ^ t0;

            uint8x16_t t1 = (temp[3*k + 3] ^ (temp[3*k+2] >> 4)) & low_nibble_mask;
            acc[3*(r*K_MAX) + 3*k + 1] ^= temp[3*k+2] ^ (t1 << 4);
            acc[3*(r*K_MAX) + 3*k + 4] ^= temp[3*k+3] ^ t1;

            uint8x16_t t2 = (temp[3*k + 5] ^ (temp[3*k+4] >> 4)) & low_nibble_mask;
            acc[3*(r*K_MAX) + 3*k + 2] ^= temp[3*k+4] ^ (t2 << 4);
            acc[3*(r*K_MAX) + 3*k + 5] ^= temp[3*k+5] ^ t2;
        }
#if K_MAX % 2 == 1
        uint8x16_t t0 = (temp[3*k + 1] ^ (temp[3*k] >> 4)) & low_nibble_mask;
        acc[3*(r*K_MAX) + 3*k] ^= temp[3*k] ^ (t0 << 4);
...
inline void mayo_3_Vt_times_Pv_neon(const uint64_t *_Pv, const uint8x16_t *V_multabs, uint64_t *_acc){
    ...
    for (size_t c = 0; c < K_MAX; c++)
    {
        ...
        {
            uint8x16_t t0 = (temp[3*k + 1] ^ (temp[3*k] >> 4)) & low_nibble_mask;
            acc[3*(k*K_MAX) + 3*c] ^= temp[3*k] ^ (t0 << 4);
            acc[3*((k+1)*K_MAX) + 3*c] ^= temp[3*k+1] ^ t0;

            uint8x16_t t1 = (temp[3*k + 3] ^ (temp[3*k+2] >> 4)) & low_nibble_mask;
            acc[3*(k*K_MAX) + 3*c + 1] ^= temp[3*k+2] ^ (t1 << 4);
            acc[3*((k+1)*K_MAX) + 3*c + 1] ^= temp[3*k+3] ^ t1;

            uint8x16_t t2 = (temp[3*k + 5] ^ (temp[3*k+4] >> 4)) & low_nibble_mask;
            acc[3*(k*K_MAX) + 3*c + 2] ^= temp[3*k+4] ^ (t2 << 4);
            acc[3*((k+1)*K_MAX) + 3*c + 2] ^= temp[3*k+5] ^ t2;
        }
#if K_MAX % 2 == 1
        uint8x16_t t0 = (temp[3*k + 1] ^ (temp[3*k] >> 4)) & low_nibble_mask;
        acc[3*k*K_MAX + 3*c] ^= temp[3*k] ^ (t0 << 4);
